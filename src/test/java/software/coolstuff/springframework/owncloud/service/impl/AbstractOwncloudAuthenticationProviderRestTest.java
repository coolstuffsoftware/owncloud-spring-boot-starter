...
public abstract class AbstractOwncloudAuthenticationProviderRestTest extends AbstractOwncloudAuthenticationProviderTest
  ...
  protected void prepareTestAuthenticate_OK(Credentials credentials, boolean enabled, String email, String displayName,
    ...
    respondUser(RestRequest.builder().method(GET).url("/cloud/users/" + credentials.getUsername())
        .basicAuthentication(credentials.getBasicAuthorizationHeader()).build(), enabled, email, displayName);
    respondGroups(RestRequest.builder().server(createServer(userDetailsService)).method(GET)
        .url("/cloud/users/" + credentials.getUsername() + "/groups").build(), groups);
  }

  @Override
  protected void prepareTestAuthenticate_NOK(Credentials credentials) throws MalformedURLException {
    respondHttpStatus(RestRequest.builder().method(GET).url("/cloud/users/" + credentials.getUsername())
        .basicAuthentication(credentials.getBasicAuthorizationHeader()).build(), HttpStatus.UNAUTHORIZED);
  }
  ...
  public void test404NotFound() throws Exception {
    ...
    respondHttpStatus(RestRequest.builder().method(GET).url("/cloud/users/" + credentials.getUsername())
        .basicAuthentication(credentials.getBasicAuthorizationHeader()).build(), HttpStatus.NOT_FOUND);
  ...
  public void testBadCredentialsByRestError() throws Exception {
    Credentials credentials = Credentials.builder().username("user1").password("password").build();
    respondFailure(RestRequest.builder().method(GET).url("/cloud/users/" + credentials.getUsername())
        .basicAuthentication(credentials.getBasicAuthorizationHeader()).build(), 999);
    authenticationProvider.authenticate(credentials.getUsernamePasswordAuthenticationToken());
  }
